...
useEffect(()=>{
  ...
  return productData? (
   ...
   {/*---------- product information---------- */}
                ...
                <div className='flex flex-col gap-4 my--8'>
                    ...
                    <div className='flex gap-2'>
                        {productData.sizes.map((item,index)=>(
                          <button key={index} className={`border border-gray-200 py-2 px-4 bg-gray-100 ${item===size?'border-orange-500':''}`} onClick={()=>setSize(item)}>{item}</button>
                        ))}
                    </div>
                </div>
                <button className='bg-black text-white mt-5 py-3 px-8 text-xl active:bg-gray-800' onClick={()=>addToCart(productData._id,size)}>Add To Cart</button>
         ...
         <div className='mt-20'>
              <div className='flex'>
                <b className='border px-5 py-3 text-xl'>Description</b>
                <p className='border px-5 py-3 text-xl'>Reviews(122)</p>
              </div>

              <div className='flex flex-col gap-4 border px-6 py-6 mt-1 text-sm text-gray-500'>
